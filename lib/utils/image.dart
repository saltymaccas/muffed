...
Future<Size> retrieveImageDimensions(String imageUrl) async {
  try {
    final isImage = isImageUrl(imageUrl);
...
Size getGIFImageDimensions(Uint8List bytes) {
  ...
  final width = (bytes[7] << 8) | bytes[6];
  final height = (bytes[9] << 8) | bytes[8];
  return Size(width.toDouble(), height.toDouble());
}
...
Size getWEBPImageDimensions(Uint8List bytes) {
  ...
  final width = (bytes[27] << 8) | bytes[26];
  final height = (bytes[29] << 8) | bytes[28];
  return Size(width.toDouble(), height.toDouble());
}
